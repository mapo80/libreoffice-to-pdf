...
sed -i '/--with-visual-studio=<2019\/2022\/2022preview>/ s|2022preview>|2022preview/2026>|' "$CONFIGURE"
...
sed -i 's/vsversions="16"/vsversions="18 17 16"/' "$CONFIGURE"

# 4. Add case for version 18.0 in find_msvc()
sed -i '/17\.0 | 17\.14)/,/vctoolset=v143/ {
    /vctoolset=v143/ a\
        ;;\
        18.0)\
            vcyear=2026\
...
sed -i 's/16\.0 | 17\.0 | 17\.14)/16.0 | 17.0 | 17.14 | 18.0)/' "$CONFIGURE"
