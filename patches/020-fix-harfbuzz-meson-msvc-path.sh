...
s|^\([[:space:]]*\)\$(MESON) setup builddir \\$|\1env -u CL -u _CL_ -u cl -u _cl_ -u CFLAGS -u CXXFLAGS -u CPPFLAGS -u LDFLAGS -u cflags -u cxxflags -u cppflags -u ldflags -u INCLUDE -u include CC="$(cross_cc_native)" CXX="$(cross_cxx_native)" $(MESON) setup builddir \\|
EOF
